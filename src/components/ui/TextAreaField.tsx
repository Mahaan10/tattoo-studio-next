...
function TextAreaField<T extends Record<string, any>>({
    label, name, register, errors, rows = 4
}: TextAreaFieldProps<T>) {
    return (
        <div className="relative">
            <textarea id={label} rows={rows} placeholder=" " {...register(name)} className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-heading bg-transparent rounded-lg border border-onyx/50 hover:border-onyx/75 focus:border-onyx transition-all duration-300 focus:outline-none peer resize-none" />
            <label
                htmlFor={label}
