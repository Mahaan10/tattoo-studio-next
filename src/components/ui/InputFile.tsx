...
function InputFile<T extends Record<string, any>>({ label, name, setValue, showPreview = true, accept = "image/*", errors, required }: InputFileProps<T>) {
        ...
        <div className="flex flex-col gap-2">
            <div className="relative">
                <input
                    ...
                    className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-heading bg-transparent rounded-base border border-onyx/50 hover:border-onyx/75 focus:border-onyx transition-all duration-300 focus:shadow-xs shadow-black-red appearance-none focus:outline-none peer min-w-lg rounded-lg"
                />
            ...
            {showPreview && previews.length > 0 && (
                <div className="flex flex-wrap gap-x-3 my-1">
                    {previews.map((url, index) => (
                        <div key={index} className="relative w-14 h-14 border border-onyx/20 rounded-md overflow-hidden my-1 group">
                            <Image src={url} alt={`Preview ${index}`} fill className="object-cover" />
                            {/* Delete btn */}
                            <button type="button" onClick={() => removeImage(index)} className="absolute -top-1 -right-1 text-red-700 bg-transparent rounded-full" title="Remove image">
